...
public class Bob {
    ...
    public byte[] Step2(byte[] step1package)
            throws IllegalStateException, IllegalBlockSizeException, BadPaddingException, NoSuchAlgorithmException {
        if (step1package.length != 8) {
            ...
            byte[] swappedArray = CommonFunctions.swap(decryptedPackage);

            //generating the new session key
            sessionKey = KeyGenerator.getInstance("DES").generateKey();

            //XOR r+1 with the new session key s
            byte[] xoredValue = CommonFunctions.XOR(swappedArray, sessionKey.getEncoded());
